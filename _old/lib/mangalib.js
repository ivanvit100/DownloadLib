...
(function (global) {
    ...
    async function _trackRequest() {
        if (global.GlobalRateLimiter) {
            await global.GlobalRateLimiter.trackRequest('mangalib');
        }
    }

    mangalib.setRateLimit = function(limit) {
        if (global.GlobalRateLimiter) {
            global.GlobalRateLimiter.setLimit(limit);
        } else {
            console.warn('[mangalib] GlobalRateLimiter not available');
        }
    };
    ...
    mangalib.splitLongImage = async function(base64Data, contentType) {
        return new Promise((resolve, reject) => {
            ...
            img.onerror = () => {
                console.warn('Ошибка загрузки изображения для разрезания');
                resolve([{ base64: base64Data, contentType }]);
            };
    ...
    mangalib.loadPageAsBase64 = async function (ref, opts = {}) {
        try {
            ...
            if (opts.splitLongImages !== false) {
                const parts = await mangalib.splitLongImage(base64Data, contentType);
    ...
    mangalib.processChapterPages = async function (pages, opts = {}, onProgress) {
        ...
        const loadPage = async (page, index) => {
            try {
                ...
                if (!img) {
                    result.push({ type: 'text', text: `[image ${index + 1} failed to load]` });
                    return;
                }
                ...
                result.push({ type: 'text', text: `[image ${index + 1} error]` });
            }
