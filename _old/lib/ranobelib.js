...
(function (global) {
    ...
    async function _trackRequest() {
        if (global.GlobalRateLimiter) {
            await global.GlobalRateLimiter.trackRequest('ranobelib');
        }
    }

    ranobelib.setRateLimit = function(limit) {
        if (global.GlobalRateLimiter) {
            global.GlobalRateLimiter.setLimit(limit);
        } else {
            console.warn('[ranobelib] GlobalRateLimiter not available');
        }
    };
    ...
    ranobelib.processChapterPages = async function (pages, opts = {}, onProgress) {
        ...
        const tasks = pages.map((page, index) => loadPage(page, index));
