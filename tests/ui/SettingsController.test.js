...
function setupDOM() {
    document.body.innerHTML = `
        ...
        <div id="serviceServersContainer"></div>
    `;
}
...
describe('_serverHintText()', () => {
    it('returns empty string for falsy serverCfg', () => {
        expect(SettingsController._serverHintText(null)).toBe('');
        expect(SettingsController._serverHintText(undefined)).toBe('');
    });

    it('returns original quality text when compress is false', () => {
        expect(SettingsController._serverHintText({ compress: false }))
            .toBe('Оригинальное качество. Сжатие на стороне расширения отключено.');
    });

    it('returns compression text when compress is not false', () => {
        expect(SettingsController._serverHintText({ compress: true }))
            .toBe('Изображения сжимаются. Сжатие на стороне расширения активно.');
        expect(SettingsController._serverHintText({}))
            .toBe('Изображения сжимаются. Сжатие на стороне расширения активно.');
    });
});

describe('_renderServiceServers()', () => {
    afterEach(() => {
        delete globalThis.serviceRegistry;
        localStorage.clear();
    });

    it('returns early when container absent', () => {
        document.getElementById('serviceServersContainer').remove();
        expect(() => SettingsController._renderServiceServers()).not.toThrow();
    });

    it('hides container when no services have imageServers', () => {
        globalThis.serviceRegistry = { getAllServices: () => [{ config: {} }] };
        SettingsController._renderServiceServers();
        expect(document.getElementById('serviceServersContainer').style.display).toBe('none');
    });

    it('creates a section per service with imageServers', () => {
        globalThis.serviceRegistry = {
            getAllServices: () => [{
                ...
                config: {
                    ...
                    imageServers: {
                        compression: { domain: 'https://img3.cdnlibs.org', label: 'Сжатие', compress: true }
                    }
                }
            }]
        };
        ...
        expect(container.querySelectorAll('.settings-section').length).toBe(1);
    });
});

describe('_createServerSection()', () => {
    afterEach(() => {
        delete globalThis.serviceRegistry;
        localStorage.clear();
        vi.useRealTimers();
    ...
    function makeService(extraServers = {}) {
        return {
            name: 'testsvc',
            config: {
                label: 'Test Service',
    ...
    it('creates section with title, select, hint and apply button', () => {
        const section = SettingsController._createServerSection(makeService());
        expect(section.className).toBe('settings-section');
        expect(section.querySelector('.settings-section-title').textContent)
            .toContain('Test Service');
        expect(section.querySelector('select.settings-select')).toBeTruthy();
        expect(section.querySelector('.settings-hint')).toBeTruthy();
        expect(section.querySelector('button.settings-btn-outline')).toBeTruthy();
    });

    it('select options match imageServers keys', () => {
        const section = SettingsController._createServerSection(
            makeService({ server1: { domain: 'https://img1.cdnlibs.org', label: 'Сервер 1', compress: false } })
        );
        const opts = [...section.querySelectorAll('select option')].map(o => o.value);
        expect(opts).toContain('compression');
        expect(opts).toContain('server1');
    });

    it('select value defaults to saved localStorage key', () => {
        localStorage.setItem('testsvc_image_server', 'compression');
        const section = SettingsController._createServerSection(makeService());
        expect(section.querySelector('select').value).toBe('compression');
    });

    it('changing select updates hint text', () => {
        const service = makeService({
            server1: { domain: 'https://img1.cdnlibs.org', label: 'Сервер 1', compress: false }
        });
        const section = SettingsController._createServerSection(service);
        const select = section.querySelector('select');
        select.value = 'server1';
        select.dispatchEvent(new Event('change'));
        expect(section.querySelector('.settings-hint').textContent)
            .toContain('Оригинальное качество');
    });

    it('apply button saves selection to localStorage', () => {
        const section = SettingsController._createServerSection(makeService());
        const select = section.querySelector('select');
        select.value = 'compression';
        section.querySelector('button').click();
        expect(localStorage.getItem('testsvc_image_server')).toBe('compression');
    });

    it('apply button shows confirmation then restores after timeout', () => {
        vi.useFakeTimers();
        const section = SettingsController._createServerSection(makeService());
    ...
    it('option textContent falls back to key when srv.label is absent', () => {
        const svc = {
            name: 'mysvc',
            config: {
                defaultImageServer: 'compression',
        ...
        const section = SettingsController._createServerSection(svc);
        const opt = section.querySelector('select option');
        expect(opt.textContent).toBe('compression');
    });

    it('saved defaults to compression when no localStorage and no defaultImageServer', () => {
        const svc = {
            name: 'mysvc',
            config: {
                imageServers: { compression: { domain: 'https://img3.cdnlibs.org', label: 'С', compress: true } }
            }
        };
        const section = SettingsController._createServerSection(svc);
        expect(section.querySelector('select').value).toBe('compression');
    ...
    it('uses name as title when config.label is absent', () => {
        const svc = {
            name: 'mysvc',
            config: {
                defaultImageServer: 'compression',
        ...
        const section = SettingsController._createServerSection(svc);
        expect(section.querySelector('.settings-section-title').textContent).toContain('mysvc');
    });
});
