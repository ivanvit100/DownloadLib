...
(function (global) {
    ...
    lib.openDownloadTabWithBase64 = async function (base64Data, filename) {
        ...
        try {
            ...
            try {
                ...
                console.error('openDownloadTabWithBase64 failed:', e, e2);
                return false;
            }
        }
    };
    ...
    lib.startDownload = async function (contentString, filename, status) {
        try {
            if (status) status.textContent = 'Отправляем в background для скачивания...';
            const resp = await lib.sendRuntimeMessage({ action: 'downloadFb2', filename, content: contentString });
            if (resp && resp.ok) return { ok: true, method: 'background' };
            else console.warn('background response:', resp);
        } catch (e) {
            console.warn('sendRuntimeMessage to background failed:', e);
        }

        try {
            if (status) status.textContent = 'Попытка downloads API в popup...';
            const blob = new Blob([contentString], { type: 'application/fb2;charset=utf-8' });
            ...
            if (typeof browser !== 'undefined' && browser.downloads && browser.downloads.download) {
                try {
                    ...
                    setTimeout(() => { try { URL.revokeObjectURL(objectUrl); } catch (e) {} }, 2 * 60 * 1000);
                    return { ok: true, method: 'downloads-popup' };
                } catch (e) {
                    console.warn('browser.downloads.download failed in popup:', e);
                    try { URL.revokeObjectURL(objectUrl); } catch (_) {}
                }
            }
            if (typeof chrome !== 'undefined' && chrome.downloads && chrome.downloads.download) {
                const res = await new Promise((resolve) => {
                    chrome.downloads.download({ url: objectUrl, filename, saveAs: true }, (id) => {
                ...
                if (res.ok) return { ok: true, method: 'downloads-popup' };
                console.warn('chrome.downloads.download failed in popup:', res.error);
                try { URL.revokeObjectURL(objectUrl); } catch (_) {}
            }
        } catch (e) {
            console.warn('downloads API attempt in popup failed:', e);
        }

        try {
            if (status) status.textContent = 'Фолбек: открываем вкладку с data:URL...';
            ...
            console.warn('data-tab fallback failed:', e);
        ...
        let fb2 = `
            ...
            <FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0" xmlns:l="http://www.w3.org/1999/xlink">
                <description>
                    ...
                    <document-info>
                        <author>
                            <nickname>Manga Downloader Extension</nickname>
                        </author>
                        <date>${ new Date().toISOString().split('T')[0] }</date>
                        <program-used>Manga to FB2 Downloader</program-used>
                    </document-info>
                </description>`;
    ...
    lib.downloadManga = async function (mangaSlug, contentType, status, progress, service) {
        ...
        // Если передана service с методами fetchMangaMetadata — используем её
        if (service && service.fetchMangaMetadata && service.fetchChaptersList && service.fetchChapter) {
            console.log('Using provided service (mangalib)');
            ...
            if (manga.cover && manga.cover.default) {
                try {
                    ...
                    const coverResponse = await fetch(manga.cover.default);
                    if (coverResponse.ok) {
                        const coverBlob = await coverResponse.blob();
                        coverBase64 = await lib.blobToBase64(coverBlob);
                        if (status) status.innerHTML += `<span style="color: green;"> ✓</span>`;
                    } else {
                        if (status) status.innerHTML += `<span style="color: orange;"> ⚠</span>`;
                    }
                } catch (e) {
        ...
        // Fallback для ranobelib — оригинальный рабочий код
        ...
        if (!mangaResponse.ok) {
            ...
            console.error('Request URL:', mangaUrl);
        ...
        const chaptersResponse = await fetch(chaptersUrl, {
            headers: { 'Accept': '*/*', 'Site-Id': '3', 'Content-Type': 'application/json' }
        });
