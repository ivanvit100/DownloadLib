...
(function (global) {
    ...
    mangalib.resolvePageUrl = function (filename) {
        ...
        // Если уже полный URL — возвращаем сразу
        if (/^https?:\/\//i.test(filename)) {
            return filename;
        }
    
        // Если начинается с / - это относительный путь от корня
        if (filename.startsWith('/')) {
            return `${IMAGE_DOMAIN}${filename}`;
        }
    
        // Иначе просто добавляем домен и /
        return `${IMAGE_DOMAIN}/${filename}`;
    };
    ...
    mangalib.loadPageAsBase64 = async function (ref, opts = {}) {
        try {
            ...
            if (typeof ref === 'string') {
                ...
                url = ref.url;
                // Проверяем, нужно ли резолвить URL
                if (!/^https?:\/\//i.test(url)) {
    ...
    mangalib.processChapterPages = async function (pages, opts = {}, onProgress) {
        ...
        for (const p of pages) {
            i++;
            try {
                const img = await mangalib.loadPageAsBase64(p, opts);
                if (img) {
                    result.push({
                        type: 'image',
                        id: `manga_img_${Date.now()}_${i}`,
                        data: img
                    });
                } else {
                    result.push({ type: 'text', text: `[image ${i} failed to load]` });
                }
            } catch (e) {
